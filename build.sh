...
case "$TRAVIS_OS_NAME" in
  ...
  "osx")
    ...
    osslsigncode sign -t http://timestamp.digicert.com -h sha1 -key .travis/ob1.pvk -pass $ENCRYPTION_SECRET -certs .travis/ob1.cert.spc -in dist/win32/OpenBazaar2-$PACKAGE_VERSION-Setup-32.exe -out dist/win32/OpenBazaar2-$PACKAGE_VERSION-Setup-32.exe
    osslsigncode sign -t http://timestamp.digicert.com -h sha1 -key .travis/ob1.pvk -pass $ENCRYPTION_SECRET -certs .travis/ob1.cert.spc -in dist/win32/OpenBazaar2Client-$PACKAGE_VERSION-Setup-32.exe -out dist/win32/OpenBazaar2Client-$PACKAGE_VERSION-Setup-32.exe
